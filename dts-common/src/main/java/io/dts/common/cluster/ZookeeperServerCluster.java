...
public class ZookeeperServerCluster implements ServerCluster, PathChildrenCacheListener {
    ...
    private void initIdgenerator() {
        DistributedAtomicInteger atomicInteger =
            new DistributedAtomicInteger(client, dtsServerParentNode, new ExponentialBackoffRetry(1000, 1));
    ...
    public Integer registry(int rpcPort) {
        String path =
            ZKPaths.makePath(dtsServerParentNode, NetUtil.getLocalIp() + ":" + Integer.valueOf(rpcPort).toString());
        try {
